...
public final class KafkaK8sClients {
  ...
  private static final String CONNECT_QUICKSTART = "/olm/quickstarts/rhosak-openshift-connect-quickstart.yaml";
  private static final String GETTING_STARTED_QUICKSTART = "/olm/quickstarts/rhosak-openshift-getting-started-quickstart.yaml";
  private static final String KAFKACAT_QUICKSTART = "/olm/quickstarts/rhosak-openshift-kafkacat-quickstart.yaml";
  private static final String QUARKUS_BIND_QUICKSTART = "/olm/quickstarts/rhosak-openshift-quarkus-bind-quickstart.yaml";

  private static final String[] QUICKSTARTS = {CONNECT_QUICKSTART, GETTING_STARTED_QUICKSTART, KAFKACAT_QUICKSTART, QUARKUS_BIND_QUICKSTART};
  ...
  public void initQuickStarts() {
    if (hasConsoleQuickStart()) {
      var quickstartCRD = client.apiextensions().v1beta1().customResourceDefinitions().withName("consolequickstarts.console.openshift.io").get();
      ...
      for (String quickstartFile : QUICKSTARTS) {
        try(InputStream fileStream = KafkaK8sClients.class.getClassLoader().getResourceAsStream(quickstartFile)) {
          var consoleQuickStart = quickStartClient.load(fileStream);
          var quickStartName = ((Map<String, Object>)consoleQuickStart.get("metadata")).get("name").toString();
          LOG.info(quickStartName);
          LOG.info(((List)quickStartClient.list(new ListOptionsBuilder().withKind("consolequickstarts").withFieldSelector(String.format("metadata.name=%s", quickStartName)).build()).get("items")).size());
          if (((List)quickStartClient.list(new ListOptionsBuilder().withKind("consolequickstarts").withFieldSelector(String.format("metadata.name=%s", quickStartName)).build()).get("items")).isEmpty()) {
            quickStartClient.create(consoleQuickStart);
          }
        } catch (Exception e) {
          LOG.error("Exception creating " + quickstartFile +". Please create manually if missing.", e);
        }
      }
    }
  ...
  private boolean hasConsoleQuickStart() {
    ...
    return client.apiextensions().v1beta1().customResourceDefinitions().withName("consolequickstarts.console.openshift.io").get() != null;
  }
